...
function configureSearchTools(server: McpServer, authHeaderProvider: AuthHeaderProvider, connectionProvider: AdoConnectionProvider, userAgentProvider: () => string) {
  server.tool(
    ...
    async ({ searchText, project, repository, path, branch, includeFacets, skip, top }, toolExtraContext) => {
  ...
  server.tool(
    ...
    async ({ searchText, project, wiki, includeFacets, skip, top }, toolExtraContext) => {
      const authHeader = await authHeaderProvider(toolExtraContext);
  ...
  server.tool(
    ...
    async ({ searchText, project, areaPath, workItemType, state, assignedTo, includeFacets, skip, top }, toolExtraContext) => {
      const authHeader = await authHeaderProvider(toolExtraContext);
