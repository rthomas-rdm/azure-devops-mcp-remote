...
function configureTestPlanTools(server: McpServer, _: AuthHeaderProvider, connectionProvider: AdoConnectionProvider) {
  server.tool(
    ...
    async ({ project, filterActivePlans, includePlanDetails, continuationToken }, toolExtraContext) => {
      try {
  ...
  server.tool(
    ...
    async ({ project, name, iteration, description, startDate, endDate, areaPath }, toolExtraContext) => {
      try {
        const connection = await connectionProvider(toolExtraContext);
        const testPlanApi = await connection.getTestPlanApi();
  ...
  server.tool(
    ...
    async ({ project, planId, parentSuiteId, name }, toolExtraContext) => {
      try {
        const connection = await connectionProvider(toolExtraContext);
        const testPlanApi = await connection.getTestPlanApi();
  ...
  server.tool(
    ...
    async ({ project, planId, suiteId, testCaseIds }, toolExtraContext) => {
      try {
        const connection = await connectionProvider(toolExtraContext);
  ...
  server.tool(
    ...
    async ({ project, title, steps, priority, areaPath, iterationPath, testsWorkItemId }, toolExtraContext) => {
      try {
        const connection = await connectionProvider(toolExtraContext);
  ...
  server.tool(
    ...
    async ({ id, steps }, toolExtraContext) => {
      try {
        const connection = await connectionProvider(toolExtraContext);
        const witClient = await connection.getWorkItemTrackingApi();
  ...
  server.tool(
    ...
    async ({ project, planid, suiteid }, toolExtraContext) => {
      try {
        const connection = await connectionProvider(toolExtraContext);
  ...
  server.tool(
    ...
    async ({ project, planId, continuationToken }, toolExtraContext) => {
      try {
        const connection = await connectionProvider(toolExtraContext);
        const testPlanApi = await connection.getTestPlanApi();
